import {
  ...
  const LineChart = ({ feedbackData }: LineChartProps) => {
    ...
    const options: ChartOptions<"line"> = {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: "Messages and Feedback Sentiment Over Time",
        },
      },
    };
    ...
    return (
      <div>
        <h2>Feedback Sentiment Over Time ({filteredFeedbackData.length})</h2>
        {filteredFeedbackData.length === 0 && <>No feedback from users.</>}
  
        {/* Render the negative feedback data */}
        <h3>Negative Feedback List {filteredNegativeFeedbackData.length}</h3>
